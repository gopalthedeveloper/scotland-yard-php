...
if (!isset($_SESSION['user_id'])) {
    header('Location: login.php?redirect=lobby.php?key=' . ($_GET['key'] ?? ''));
    exit();
}
...
if (!$game) {
    $_SESSION['game_error'] = "Invalid Game Code"; 
    header('Location: index.php');
    exit();
}
$gameId = $game['id'];
...
?>
        ...
        <div class="board-main">
            <h2>Game Lobby - <?= htmlspecialchars($game['game_name']) ?></h2>
...
<script>
    ...
    function showAlert(message, type) {
        ...
        const container = document.querySelector('.alert-container') || document.querySelector('.container') || document.body;
